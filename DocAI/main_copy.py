...
# Add LibreOffice Python path for UNO bindings
...
from flask import Flask, request, Response, send_from_directory, jsonify, send_file, abort, redirect, url_for
...
FEATURES = {
    'native_viewer': True,   # Enable native viewer feature
    'debug_mode': True      # Enable logging for development
}
...
agent_status = {
    'docai': {'initialized': False, 'status': 'starting'},
    'browser': {'initialized': False, 'status': 'not_started', 'process': None},
    'social': {'initialized': False, 'status': 'not_implemented'},
    'web': {'initialized': False, 'status': 'not_implemented'},
    'service': {'initialized': False, 'status': 'not_implemented'},
    'twitter': {'initialized': False, 'status': 'not_implemented'}
}
...
@app.route('/')
def index():
    response = send_from_directory(app.static_folder, 'indexf.html')
...
# Serve static files directly from root for compatibility
@app.route('/<path:filename>')
def serve_root_static(filename):
    """Serve static files from root path"""
    # Only serve known static file types
    if filename.endswith(('.js', '.css', '.png', '.jpg', '.jpeg', '.gif', '.ico', '.html')):
        try:
            return send_from_directory(app.static_folder, filename)
        except:
            pass
    # For API routes, let them pass through
    if filename.startswith('api/') or filename.startswith('rag/'):
        # This will trigger 404 handler properly for missing API routes
        abort(404)
...
def hello():
    ...
    return jsonify({'message': 'Hello from Durga AI backend!'})

@app.route('/api/debug', methods=['GET'])
def debug_info():
    """Debug endpoint to check API configuration"""
    groq_key_set = bool(os.environ.get("GROQ_API_KEY"))
    groq_test_result = "unknown"
    groq_error = None
    ...
    return jsonify({
        ...
        'groq_api_error': groq_error,
        'rag_documents': len(rag_handler.document_store) if rag_handler else 0,
        'models_available': ['llama-3.3-70b-versatile', 'deepseek-r1-distill-llama-70b']
    })
            ...
            for chunk in stream:
                if chunk and chunk.choices and chunk.choices[0].delta:
                    if chunk.choices[0].delta.content is not None:
                        ...
                        yield f"data: {json.dumps({'text': decoded_content})}\n\n"
...
@app.route('/view_document_native/<path:filename>')
def view_document_native(filename):
    """
    Native document viewer using LibreOffice
    This is a NEW route - does not replace view_document()
    """
    if not is_feature_enabled('native_viewer'):
        # Fallback to HTML viewer if feature disabled
        logger.info(f"Native viewer disabled, redirecting to HTML viewer for: {filename}")
    ...
    try:
        ...
        # Initialize native viewer service
        from app.services.libreoffice_native_viewer import LibreOfficeNativeViewer
        viewer = LibreOfficeNativeViewer()
        
        # Check if LibreOffice is available
        if not viewer.available:
            logger.warning("LibreOffice not available, falling back to HTML viewer")
            return redirect(url_for('view_document', filename=filename))
        
        # Get document info
        doc_info = viewer.get_document_info(document_path)
        
        # Check if format is compatible
        if not doc_info.get('native_viewer_compatible'):
            logger.warning(f"Format not compatible with native viewer: {doc_info.get('format')}")
            return redirect(url_for('view_document', filename=filename))
        
        # Convert to PDF for native viewing
        pdf_path = viewer.convert_to_pdf(document_path)
        
        if pdf_path and os.path.exists(pdf_path):
            # Return PDF file for native viewing
            return send_file(
                pdf_path,
                mimetype='application/pdf',
                as_attachment=False,
            ...
            logger.error("PDF conversion failed, falling back to HTML viewer")
            return redirect(url_for('view_document', filename=filename))
        
    except Exception as e:
        logger.error(f"Native viewer error: {str(e)}")
        # Fallback to HTML viewer on error
        return redirect(url_for('view_document', filename=filename))
...
def start_browser_agent():
    """Start the browser agent in a separate process"""
    try:
        logger.info("Starting Browser Agent...")
        agent_status['browser']['status'] = 'starting'
        
        # Path to browser agent launch script
        browser_agent_path = '/media/erl/New Volume/ai_agent/BROWSER AGENT/docai_final/AGENT_B/launch_offline.sh'
        
        if not os.path.exists(browser_agent_path):
            logger.error(f"Browser agent launch script not found at {browser_agent_path}")
            agent_status['browser']['status'] = 'error'
            agent_status['browser']['initialized'] = False
            return
        
        # Make sure script is executable
        os.chmod(browser_agent_path, 0o755)
        ...
        agent_status['browser']['process'] = process
        ...
        if process.poll() is None:
            ...
            agent_status['browser']['initialized'] = True
            agent_status['browser']['status'] = 'running'
        else:
            logger.error("Browser Agent failed to start")
            agent_status['browser']['status'] = 'error'
            agent_status['browser']['initialized'] = False
            
    except Exception as e:
        logger.error(f"Error starting Browser Agent: {str(e)}")
        agent_status['browser']['status'] = 'error'
        agent_status['browser']['initialized'] = False
...
def get_agent_status():
    ...
    # Create a clean copy without the process object
    status_copy = {
        'docai': dict(agent_status['docai']),
        'browser': {
            'initialized': agent_status['browser']['initialized'],
            'status': agent_status['browser']['status']
        },
        'social': dict(agent_status['social']),
        'web': dict(agent_status['web']),
        'service': dict(agent_status['service']),
        'twitter': dict(agent_status['twitter'])
    }
    return jsonify(status_copy)

@app.route('/api/start-browser-agent', methods=['POST'])
def api_start_browser_agent():
    """API endpoint to start browser agent"""
    if agent_status['browser']['initialized']:
        return jsonify({'message': 'Browser agent already running'})
    
    # Start in a separate thread to avoid blocking
    thread = threading.Thread(target=start_browser_agent)
    thread.daemon = True
    thread.start()
    
    return jsonify({'message': 'Browser agent starting...'})

if __name__ == '__main__':
    app.chat_histories = {}
    
    # Mark DocAI as initialized when Flask starts successfully
    def mark_docai_initialized():
        time.sleep(2)  # Wait for Flask to fully start
        agent_status['docai']['initialized'] = True
        agent_status['docai']['status'] = 'running'
        logger.info("DocAI Agent initialized successfully")
        
        # Auto-start browser agent after DocAI is ready
        logger.info("Auto-starting Browser Agent...")
        start_browser_agent()
    
    # Start initialization in background
    init_thread = threading.Thread(target=mark_docai_initialized)
