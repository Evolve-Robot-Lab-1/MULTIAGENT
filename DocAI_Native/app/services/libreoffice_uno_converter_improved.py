...
class ImprovedLibreOfficeConverter:
    ...
    def _find_free_port(self):
        ...
        try:
            ...
            logger.error(f"Failed to get any port: {e}")
            return 2002  # Final fallback
    ...
    def _start_libreoffice_service(self):
        ...
        logger.info(f"Attempting to start LibreOffice on port {self.port}")
        # self._kill_existing_libreoffice()  # Commented out to prevent interference
        ...
        # Simplified command - use soffice instead of libreoffice, minimal flags
        cmd = [
    ...
    def _is_bold_weight(self, char_weight):
        """Check if CharWeight enum indicates bold formatting"""
        try:
            # CharWeight is a UNO enum, need to handle it properly
            if hasattr(char_weight, 'value'):
                # If it's an enum with .value attribute
                return char_weight.value > 100
            else:
                # Try to get numeric value directly
                weight_value = float(char_weight)
                return weight_value > 100
        except (AttributeError, ValueError, TypeError):
            # Fallback: check if it contains 'BOLD' in string representation
            return 'BOLD' in str(char_weight).upper()
    
    def _is_italic_posture(self, char_posture):
        """Check if CharPosture enum indicates italic formatting"""
        try:
            # CharPosture is a UNO enum, need to handle it properly  
            if hasattr(char_posture, 'value'):
                # If it's an enum with .value attribute
                return char_posture.value > 0
            else:
                # Try to get numeric value directly
                posture_value = float(char_posture)
                return posture_value > 0
        except (AttributeError, ValueError, TypeError):
            # Fallback: check if it contains 'ITALIC' in string representation
            return 'ITALIC' in str(char_posture).upper()
                    ...
                    for i in range(min(para_styles.getCount(), 50)):  # Limit to prevent overload
                        ...
                        if hasattr(style, 'Name') and hasattr(style, 'DisplayName'):
                            style_info = {
                                ...
                                'bold': self._is_bold_weight(style.CharWeight) if hasattr(style, 'CharWeight') else False,
                                'italic': self._is_italic_posture(style.CharPosture) if hasattr(style, 'CharPosture') else False
                            }
    ...
    def cleanup(self):
        """Clean up resources - only our process, not all LibreOffice"""
        try:
            # 1. Terminate only our LibreOffice process
            if self.lo_process:
                try:
                    logger.info(f"Terminating our LibreOffice process (PID: {self.lo_process.pid})")
                    self.lo_process.terminate()
                    self.lo_process.wait(timeout=5)
                    logger.info("LibreOffice process terminated gracefully")
                except subprocess.TimeoutExpired:
                    logger.warning("LibreOffice process didn't terminate, forcing kill")
                    self.lo_process.kill()
                    self.lo_process.wait()
                except Exception as e:
                    logger.warning(f"Error terminating LibreOffice process: {e}")
            
            # 2. DO NOT kill all LibreOffice processes - removed aggressive cleanup
            # self._kill_existing_libreoffice()  # This was killing ALL LibreOffice!
            ...
            # Don't raise, just log - cleanup errors shouldn't fail the conversion
            logger.warning(f"Non-critical cleanup warning: {str(e)}")
...
def render_document_with_uno_images(document_path):
    ...
    try:
        ...
        if result:
            ...
            # Store the successful result before cleanup
            final_result = result
        else:
            final_result = {'success': False, 'error': 'Conversion failed after retries'}
    except Exception as e:
        logger.error(f"Conversion exception: {str(e)}")
        final_result = {'success': False, 'error': f'Conversion exception: {str(e)}'}
    finally:
        # Cleanup errors should not affect the conversion result
        try:
            converter.cleanup()
        except Exception as cleanup_error:
            logger.warning(f"Cleanup warning (non-critical): {cleanup_error}")
    
    # Return the conversion result regardless of cleanup issues
    return final_result
