# PyWebView API Error Analysis & Resolution

## Problem Summary
The persistent "RuntimeError: unable to get the value" error occurs when PyWebView cannot properly serialize or access the API methods. This has been happening across multiple test implementations.

## Root Causes Identified

### 1. PyWebView Cache Corruption
PyWebView maintains a cache that can become corrupted between runs, especially during development when API structures change frequently. The main app clears this cache before window creation.

### 2. API Object Structure Requirements
PyWebView has very specific requirements:
- Must be a class instance (not dictionary or module)
- Methods must be simple function references (not nested functions or lambdas)
- No complex return types in __init__ or property getters

### 3. Window Reference Timing
The window reference must be set AFTER window creation, not before. Setting it too early or in the wrong context causes serialization issues.

## Working Pattern from DocAI Native

```python
# 1. Module-level functions (simple, no closures)
def ping():
    return {"pong": True}

# 2. Class with simple method assignments
class FixedAPI:
    def __init__(self):
...
# 3. Clear cache before window creation
import shutil
cache_paths = [Path.home() / '.pywebview', ...]
for path in cache_paths:
    if path.exists():
        shutil.rmtree(path)

# 4. Create window with class instance
api = FixedAPI()
window = webview.create_window(..., js_api=api)

# 5. Set window reference AFTER creation
api.set_window(window)
```

## Why Previous Tests Failed

### test_overlay_approach.py
- Used nested functions returning dictionaries
- Complex object structures in API methods

### test_simple_overlay.py  
- Used dictionary API instead of class instance
- Global variables causing serialization issues

### test_final_overlay.py
- Methods defined inside class (not module-level)
- Missing cache clearing step

### test_working_overlay.py
- Close to working but missing cache clear
- May have stale cache from previous runs
